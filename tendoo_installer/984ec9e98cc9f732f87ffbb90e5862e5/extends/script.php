<?php 
class oldal_tendoo_theme_handler
{
	public $news;	
	public $file;
	private $data;
	...
	*/
	public function __construct($data)
	{
		...
		// Définition du chemin d'accès vers les fichiers css
		$this->file->css_url					=	$this->url->main_url().THEMES_DIR.$this->theme['ENCRYPTED_DIR'].'/css';
        // Définition du chemin d'accès vers les fichiers javascript
        $this->file->js_url						=	$this->url->main_url().THEMES_DIR.$this->theme['ENCRYPTED_DIR'].'/js';
        
/*PUSH_GLOBAL_JS*/
		css_push_if_not_exists("css/globals/style.css");
		css_push_if_not_exists("css/globals/mCSB_buttons.html");
		css_push_if_not_exists("css/globals/jquery.selectBoxIt.css");
		css_push_if_not_exists("css/globals/jquery.mCustomScrollbar.css");
		css_push_if_not_exists("css/globals/jquery.fancybox.css");
		css_push_if_not_exists("css/globals/flexslider.css");
		css_push_if_not_exists("css/globals/entypo.css");
		css_push_if_not_exists("css/globals/bootstrap.min.css");
	...
	**/
	public function header($data)
	{
		...
		$this->load->view('tendoo_themes/'.$this->theme['ENCRYPTED_DIR'].'/default/header',$this->data,FALSE,TRUE);
	}
	public function head($data)
	{
		$this->data	=	array_merge($this->data,$data);
		return $this->load->view('tendoo_themes/'.$this->theme['ENCRYPTED_DIR'].'/default/head',$this->data,true,TRUE);
	}
	public function footer($data)
	{
		$this->data	=	array_merge($this->data,$data);
		return $this->load->view('tendoo_themes/'.$this->theme['ENCRYPTED_DIR'].'/default/footer',$this->data,true,TRUE);
	}
	public function body($data)
	{
		...
		$this->load->view('tendoo_themes/'.$this->theme['ENCRYPTED_DIR'].'/default/body',$this->data,FALSE,TRUE);
	}
	/**
		Méthode : Pagination.
	**/
...
/*PUSH_LASTESTELEMENT_CSS_IF_NOT_EXISTS*/
	...
	*/
	...
	public function defineBlogPost($title,$content,$thumb,$full,$author,$link,$timestamp,$categories	=	array(),$comments	=	false)
	{
...
/*PUSH_BLOGPOST_CSS_IF_NOT_EXISTS*/
	...
	*/
	...
	public function defineSingleBlogPost($title,$content,$thumb,$full,$author,$timestamp,$categories	=	array())
	{
		...
		if(array_key_exists('type',$a))
		{
			...
			else
			{
				if(array_key_exists('subtype',$a))
				{
					if(in_array($a['subtype'],array('submit','reset','button')))
					{
					$balise	=	'<div class="clearfix"></div><input '.$name.' '.$subtype.' '.$value.'/>';
					}
					else
					{
